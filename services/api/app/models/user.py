...
class User(Base):
    ...
    ltc_wallet_address: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    # Auto-payout threshold: auto-withdraw when balance exceeds this (0 = disabled)
    auto_payout_threshold: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
    # Security hold: withdrawals blocked until this timestamp (set on password/email change)
    security_hold_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    pool_profiles = relationship("PoolProfile", back_populates="user")
